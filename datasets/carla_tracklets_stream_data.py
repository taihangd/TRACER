    ...
    def __init__(self, 
        ...
        with open(record_file, 'r') as f: # the license plate text is not considered currently.
            for l in tqdm(f):
                ...
                plate_feats_list.append(r['plate_feature'] if use_plate and r['plate_feature'] is not None else np.zeros(2048)) # stay consistent with original features
        ...
        lon_arr = np.array([cameras_xy[cam_id][0] for cam_id in self.camera_ids_arr], dtype=np.float32)
        lat_arr = np.array([cameras_xy[cam_id][1] for cam_id in self.camera_ids_arr], dtype=np.float32)
