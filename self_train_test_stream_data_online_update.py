...
from datasets.urban_vehicle_stream_data import *
from datasets.carla_tracklets_stream_data import *
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cfg_file', 
                        ...
                        default='./config/uv_self_train_stream_data_online_update.yaml')
    ...
    if args.dataset == "UrbanVehicle":
        test_dataset = uv_img_dataset(
            ...
            train_time_win_size=args.train_time_win_size, 
            fps=args.fps,
        )
    ...
    gen_cluster_multimodal_feat_total_time = 0
    get_sample_idx_pairs_total_time = 0
    network_update_total_time = 0
    ...
    for batch_idx, (curr_batch_car_feats, 
        ...
        curr_batch_st_feats = extract_st_feat_from_tensor(curr_batch_camera_ids, 
                                                          curr_batch_time, 
                                                          curr_batch_lon, 
                                                          curr_batch_lat, 
                                                          model, 
                                                          device, 
                                                          cam_vec_idx_dict, 
                                                          road_graph_node_emb) # extract spatio-temporal features
        ...
        curr_batch_pred_labels = pred_labels[prev_processed_record_num:prev_processed_record_num+len(curr_batch_car_feats)]
        if args.dataset == 'Carla':
            precision, recall, fscore, expansion, _ = eval.evaluate_snapshots(curr_batch_gt_label.numpy(), 
                                                                              curr_batch_pred_labels, 
        ...
        # organize and print evaluation time
        ...
        # determine whether it is the last batch of data and whether to skip network update
        if batch_idx == len(extract_feats_data_loader) - 1:
            break
        ...
        label_idx_dict = gen_cluster_multimodal_feat(curr_batch_st_feats, 
                                                    curr_batch_car_feats.numpy(), 
                                                    curr_batch_plate_feats.numpy(), 
                                                    curr_batch_pred_labels, 
                                                    device, 
                                                    args.temperature, 
                                                    alpha=args.alpha)
        gen_cluster_multimodal_feat_time = time.time() - gen_cluster_multimodal_feat_time
        gen_cluster_multimodal_feat_total_time += gen_cluster_multimodal_feat_time
        print(f'generate cluster multi-modal feature consume time: {gen_cluster_multimodal_feat_time}')

        # generate a list of sample pairs
        get_sample_idx_pairs_time = time.time()
        # generate the parameters to control sampling
        curr_batch_pred_traj_num = len(set(curr_batch_pred_labels))
        num_sampl_group = int(curr_batch_pred_traj_num * args.sampl_group_num_coef / args.batch_size)
        num_group_pairs = max(int(len(curr_batch_car_feats) / curr_batch_pred_traj_num), 2) # set the pairs number in the group
        print(f'batch {batch_idx} the number of sample group: {num_sampl_group}')
        print(f'batch {batch_idx} the number of group pairs: {num_group_pairs}')
        all_pairs_list = get_sample_idx_pairs(curr_batch_pred_labels, num_sampl_group, num_group_pairs, args.batch_size)
        get_sample_idx_pairs_time = time.time() - get_sample_idx_pairs_time
        get_sample_idx_pairs_total_time += get_sample_idx_pairs_time
        print(f'get sample idx pairs consume time: {get_sample_idx_pairs_time}')

        # train model
        network_update_time = time.time()
        if args.preload_gpu_flag:
            curr_batch_car_feats = curr_batch_car_feats.to(device)
            curr_batch_plate_feats = curr_batch_plate_feats.to(device)
            curr_batch_time = curr_batch_time.to(device)
            curr_batch_lon = curr_batch_lon.to(device)
            curr_batch_lat = curr_batch_lat.to(device)
            curr_batch_camera_ids = curr_batch_camera_ids.to(device)
        loss_epoch = train(model, optimizer, 
                            all_pairs_list, 
                            curr_batch_car_feats, curr_batch_plate_feats, curr_batch_time,
                            curr_batch_lon, curr_batch_lat, curr_batch_camera_ids,
                            np.array(curr_batch_pred_labels), label_idx_dict, road_graph_node_emb, 
                            ...
                            curr_batch_time[0] + args.test_time_interval * args.fps, curr_batch_time[0] + 2 * args.test_time_interval * args.fps, 
        ...
        network_update_total_time += network_update_time
        print(f'network update consume time: {network_update_time}')
        print(f"Loss: {loss_epoch / len(all_pairs_list)}")
        
        print(f'current batch data totally consumes time: {extract_feat_time + cluster_time + gen_cluster_multimodal_feat_time + get_sample_idx_pairs_time + network_update_time}')
    ...
    print(f'generate cluster multimodal features total consumes time: {gen_cluster_multimodal_feat_total_time}')
    print(f'get sample idx pairs total consumes time: {get_sample_idx_pairs_total_time}')
    print(f'network update total consumes time: {network_update_total_time}')
    print(f'evaluation total consumes time: {eval_total_time}')
    print(f'trajectory recovery total consumes time: {extract_feat_total_time + cluster_total_time + gen_cluster_multimodal_feat_total_time + get_sample_idx_pairs_total_time + network_update_total_time}')
