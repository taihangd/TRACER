...
class SpatioTemporalProjector(nn.Module):
    def __init__(self, batch_size, time_feat_dim, 
        ...
        if road_graph_node_vec is not None:
            num_inputs = road_graph_node_vec.shape[1] + self.time_feat_dim + 3
        else:
            num_inputs = 3
        num_hiddens = 256
    ...
    def forward(self, st_info, road_graph_node_vec=None):
        tm, _, _ = torch.split(st_info, 1, dim=1)
        coef = torch.arange(1, self.time_feat_dim + 1).to(self.device) * self.time_scaling_factor
        time_feat = tm * coef
        din = torch.cat((road_graph_node_vec, time_feat, st_info), dim=-1)
        dout = self.fc(din)

        return dout
    
# for convenience to switch different encoder models
def get_st_proj(batch_size, time_feat_dim, time_scaling_factor, mapped_feat_dim, road_graph_node_vec, device, name):
    st_mlp_proj = SpatioTemporalProjector(batch_size, time_feat_dim, time_scaling_factor, mapped_feat_dim, road_graph_node_vec, device) # default encoder

    st_proj_dict = {
        "MLP_emb": st_mlp_proj,
    }
    if name not in st_proj_dict.keys():
        raise KeyError(f"{name} is not a valid encoder version")
...
class Network(nn.Module): # specific case of the MoCo architecture
    ...
    def forward(self, st_info_i, st_info_j, node_vec_i, node_vec_j):
        ts_feat_i = self.st_proj(st_info_i, node_vec_i)
        ts_feat_j = self.st_proj(st_info_j, node_vec_j)
...
class Network_MoCo(nn.Module): # MoCo architecture: a query encoder, a key encoder, and a queue
    ...
    def forward(self, st_info_i, st_info_j, node_vec_i, node_vec_j):
        ...
        q = self.encoder_q(st_info_i, node_vec_i)  # a batch of query images: NxC
        if self.mlp:
            q = self.mlp_projector_q(q)  # nonlinear transformation
        q = nn.functional.normalize(q, dim=1)
        with torch.no_grad():  # no gradient to keys
            self._momentum_update_key_encoder()  # update the key encoder
            k = self.encoder_k(st_info_j, node_vec_j)  # keys: NxC
            if self.mlp:
    ...
    def forward_extract_feat(self, st_info, node_vec):
        # compute query features
        ts_feat = self.encoder_k(st_info, node_vec)  # keys: NxC
        h = normalize(ts_feat, dim=1)

        return h
    
    def forward_extract_feat_gx(self, st_info, node_vec):
        # compute query features
        ts_feat = self.encoder_k(st_info, node_vec)  # keys: NxC
        if self.mlp:
