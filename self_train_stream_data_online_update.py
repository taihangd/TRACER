...
if __name__ == "__main__":
    ...
    if args.preload_gpu_flag:
        car_feats_tensor = torch.from_numpy(car_feats).to(device)
        plate_feats_tensor = torch.from_numpy(plate_feats).to(device)
        time_tensor = torch.from_numpy(time_arr).to(device)
        lon_tensor = torch.from_numpy(lon_arr).to(device)
        lat_tensor = torch.from_numpy(lat_arr).to(device)
        camera_ids_tensor = torch.from_numpy(camera_ids_arr).to(device)
    else:
        car_feats_tensor = car_feats
        plate_feats_tensor = plate_feats
        time_tensor = time_arr
        lon_tensor = lon_arr
        lat_tensor = lat_arr
        camera_ids_tensor = camera_ids_arr
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        loss_epoch = train(model, optimizer, 
                            all_pairs_list, 
                            car_feats_tensor, plate_feats_tensor, time_tensor,
                            lon_tensor, lat_tensor, camera_ids_tensor,
                            ...
                            time_arr[0], time_arr[0] + args.train_time_win_size * args.fps, 
